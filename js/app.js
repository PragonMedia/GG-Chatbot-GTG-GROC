...
// -----------------------------------------------------------------------------
// plancompared.com redirect: click on .phone-number (or #phone-number), wait 7s,
// then redirect only if URL has key=X184GA + non-empty mb AND (mobile viewport OR mobile UA).
// -----------------------------------------------------------------------------
(function () {
  var PLANCOMPARE_BASE = "https://plancompared.com/";
  var REDIRECT_DELAY_MS = 7000;
  var KEY_REQUIRED = "X184GA";
  ...
  function passesKeyAndMbGate() {
    var params = new URLSearchParams(window.location.search);
    if (params.get("key") !== KEY_REQUIRED) return false;
    var mb = params.get("mb");
    if (mb == null || String(mb).trim() === "") return false;
    return true;
  }

  function schedulePlancompareRedirect() {
    ...
    redirectTimerId = setTimeout(function () {
      ...
      if (!passesKeyAndMbGate()) return;
      var q = window.location.search || "";
      window.location.href =
        PLANCOMPARE_BASE.replace(/\/?$/, "/") + (q || "");
      // e.g. https://plancompared.com/?key=X184GA&mb=foo
    }, REDIRECT_DELAY_MS);
  }

  function isPhoneRedirectTarget(el) {
    if (!el || el.nodeType !== 1) return false;
    if (el.id === "phone-number") return true;
    if (el.classList && el.classList.contains("phone-number")) return true;
    return false;
  }
  ...
  document.addEventListener(
    ...
    function (e) {
      ...
      while (el && el !== document.body) {
        if (isPhoneRedirectTarget(el)) {
          schedulePlancompareRedirect();
          return;
        }
        el = el.parentElement;
      }
